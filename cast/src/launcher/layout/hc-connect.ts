...
const translateErr = (err) =>
  err === ERR_CANNOT_CONNECT
    ...
    : err === ERR_HASS_HOST_REQUIRED
      ? "Please enter a Usync Assistant URL."
      : err === ERR_INVALID_HTTPS_TO_HTTP
        ? html`
            Cannot connect to Usync Assistant instances over "http://".
            ${seeFAQ("https")}
          `
        : `Unknown error (${err}).`;
...
const INTRO = html`
  <p>
    Usync Assistant Cast allows you to cast your Usync Assistant installation to
...
export class HcConnect extends LitElement {
      ...
      return html`
        <hc-layout>
          <div class="card-content">
            ${INTRO}
            <p>
              To get started, enter your Usync Assistant URL and click
              authorize. If you want a preview instead, click the show demo
              button.
            </p>
            <ha-textfield
              label="Usync Assistant URL"
  ...
  private async _handleConnect() {
    ...
    if (value === "") {
      this.error = "Please enter a Usync Assistant URL.";
      return;
    }
    if (value.indexOf("://") === -1) {
      this.error =
